...
function resolveDependencies(require, hash, resultList, seenList) {
        ...
        if (err) {
          ...
          async.detect(results, function (item, callback) {
            ...
            if (base) {
              ...
              inputs.forEach(function (inputFile) {
                console.log(inputFile);
                console.log(searchHash);
                var input = searchHash[inputFile];
                console.log(input);
                if (!input.base) {
                  seenList.push(input.path);
                  input.require.forEach(function (require) {
                    resolveDependencies(require, searchHash, resultList, seenList);
                  });
                  resultList.push(input.path);
                }
              });
